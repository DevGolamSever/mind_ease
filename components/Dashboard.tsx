...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  useEffect(() => {
    const storedBadges = JSON.parse(
      localStorage.getItem("mind_ease_badges") || "{}",
    );
    console.log(storedBadges,"<<<<<<<");

    setBadges(storedBadges[user.id] || 0);
  }, [user.id]);
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            Track your wellness journey, {user.name}.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800">{badges}</p>
          </div>
        </div>
      </div>
            ...
            <div className="relative p-6 text-center">
              ...
              <div className="mt-6 flex gap-2">
                ...
                <Button
                  fullWidth
                  variant="outline"
                  onClick={() => handleDeletMode(selectedEntry?._id)}
                >
                  Delete Mode
                </Button>
              </div>
